...
Not yet flashed. Verdict: unproven **as an artifact**; the code in it is proven. Identical image code to `1.0.0-alpha.27`, which cut a real piece on 2026-09-01. The name changes because the appliance did the job it exists to do, not because anything in the build changed.

- Changed: nothing under `stage-kiosk/`, `build.sh` or `config/`. `git diff 1.0.0-alpha.27 1.0.0-beta.1 -- stage-kiosk/ build.sh config/` is empty by construction.
- Proved: nothing of its own yet. Every claim behind this name was settled on `1.0.0-alpha.27`.
- Regressed: unknown until flashed.

A rebuild is not the same image. `1.0.0-alpha.25` and `1.0.0-alpha.27` were built from byte-identical source and their images differ by 1.1 MB, because each build resolves whatever Debian packages are current that day. So this artifact has never been on hardware even though its source has. Flash it and re-run T4 and T6 — two commands — before treating it as proven.

Known open at the time of naming: a short press on the power button does nothing and the cause is not established; rendering on large jobs is untested, the piece cut on alpha.27 having come from a 44 KB file; the ten-cut power test stands at one cut, which passed; the autologin console has never been tried; and a card carrying only `authorized_keys` cannot be tested on a rig with no Ethernet.
...
- Proved: a real job was loaded and cut on 2026-09-01 from a 44 KB file, with Carbide Motion recording zero restarts across 83 minutes of uptime and the supply reporting no throttling at 33.4°C; first boot completes and self-reboots; `/data` is a real read-write ext4 mount of the 230 GB partition — `226G` free, not `3.9G` of RAM — while `/` stays an overlay; a file written over SMB lands on `/dev/mmcblk0p3`, is readable by the `kiosk` account the session runs as, and survives a reboot byte-identical; the journal and the SSH host key survive with it, the fingerprint unchanged across an abrupt power cut, and ext4 recovered its journal cleanly. `wipefs` erased a real stale signature — `2 bytes were erased at offset 0x00000438 (ext4): 53 ef` — so the card genuinely carried the leftover that broke `1.0.0-alpha.24` and the fix was exercised rather than skipped. No 90-second device stall: the slowest unit is `carbide-kiosk-access` at 7.7s. It is also the first tag whose image could only exist because the real `overlayroot` and the real systemd were asked what they do with it; `1.0.0-alpha.26` is the proof that gate bites, having produced no release at all.
- Regressed: nothing. One long-standing symptom surfaced for the first time, because nobody had pressed the button on a flashed machine before: a short press on the power button does nothing. The cause is **not established**. What is ruled out: `HandlePowerKey=poweroff` is correct and logind is running with it, and `/dev/input/event0` carries `TAGS=:power-switch:`, so systemd's shipped rule does match the device. An earlier entry here claimed the device was mistagged; that was wrong, read off the parent node rather than the event node the rule matches, and is corrected. A 45-second capture of `/dev/input/event0` recorded no bytes, but nobody can confirm a press landed inside the window, so it settles nothing. Two candidates remain untested: `triggerhappy` runs with `--deviceglob /dev/input/event*` and holds every input node, or the Pi 5 emits no event on a short press while running, which would be platform behaviour rather than a defect in this image.

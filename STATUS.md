...
The newest release is `1.0.0-alpha.25`, tagged 2026-09-01. It has not been flashed. It is the candidate fix for the `1.0.0-alpha.24` first-boot failure and nothing about it is proven.
...
There is no image anyone should flash for real use. `1.0.0-alpha.24` does not complete first boot, `1.0.0-alpha.23` silently loses every file written to the share, and `1.0.0-alpha.25` is untested.
...
`1.0.0-alpha.25` is tagged and waiting for a clean flash. It is the candidate fix for the first-boot failure and nothing else.

`tests/machinery.sh`, the `machinery` CI job, this file and `RELEASES.md` are uncommitted in the working tree, awaiting review. They were written after `1.0.0-alpha.25` was cut, so they did not gate it. They gate everything after it.
...
1. Confirm CI is green on `1.0.0-alpha.25`, including the `machinery` job, and take the image from the release rather than from `deploy/`.
